...
class CheckoutController extends Controller
{
    ...
    public function renderCheckoutPage()
    {
        ...
        $customer_address = DB::table('customer_addresses')->where('id_customer', Auth::user()->id)->where('is_current', 1)->first(['id', 'id_city', 'id_district', 'id_ward', 'address', 'full_address']);

        $customer_addresses = DB::table('customer_addresses')->where('id_customer', Auth::user()->id)->where('id_city', 4)->get(['id', 'id_city', 'id_district', 'id_ward', 'address', 'is_current', 'is_current', 'full_address', 'is_current']);
